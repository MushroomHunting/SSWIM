    ...
    for key in all_datasets.keys():
        ...
        for experiment_id in trange(N_repeats):
            try:
                ...
                #     cfg["prescale_div"] = 0.1
                #     cfg["ls_top_div"] = 0.1
                # elif dataset_name == "bike_sharing_hourly":
                #     cfg["prescale_div"] = 0.1
                #     cfg["ls_top_div"] = 0.1
                # elif dataset_name == "ct_slice":
                #     cfg["prescale_div"] = 0.1
                #     cfg["ls_top_div"] = 0.1
                ...
                column_names = [str(itm) for itm in list(range(cfg['optim_epochs']))]
                # Save the training NLML / epoch
                result_filepath = os.path.join(log_path, f"{dataset_name}_sswim{N_warplayers}_epochs{cfg['optim_epochs']}_nlml.csv")
                df = pd.DataFrame(np.array(BLR_ui.logging["loss"]).reshape(1, -1), columns=column_names)
                with open(result_filepath, mode="a") as f:
                    df.to_csv(f, mode="a", index=False, header=f.tell() == 0)

                # Save the test RMSE / epoch
                result_filepath = os.path.join(log_path, f"{dataset_name}_sswim{N_warplayers}_epochs{cfg['optim_epochs']}_rmse.csv")
                df = pd.DataFrame(np.array(BLR_ui.logging["rmse_test_normscale"]).reshape(1, -1), columns=column_names)
                with open(result_filepath, mode="a") as f:
                    df.to_csv(f, mode="a", index=False, header=f.tell() == 0)

                # Save the test MNLP / epoch
                result_filepath = os.path.join(log_path, f"{dataset_name}_sswim{N_warplayers}_epochs{cfg['optim_epochs']}_mnlp.csv")
                df = pd.DataFrame(np.array(BLR_ui.logging["mnlp_test_normscale"]).reshape(1, -1), columns=column_names)
                with open(result_filepath, mode="a") as f:
                    df.to_csv(f, mode="a", index=False, header=f.tell() == 0)
